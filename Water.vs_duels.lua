-- Water.vs | https://discord.gg/VyRHBRfB24
...
-- ============================================================
-- INTRO SOURCE – modificato: "URANIUM.CC" → "Water.vs"
-- ============================================================
local introSource = [[
...
local function playIntro()
	...
	while tick() < glitchEnd and not introSkipped do
		...
		if math.random() < 0.35 then task.spawn(glitchText, leftLabel, "WaterVS") end
...
local COLORS = {
	...
	white = Color3.fromRGB(0, 210, 255),
	textDim = Color3.fromRGB(0, 210, 255),
...
-- ============================================================
_G.WaterVS_ThemeColor = Color3.fromRGB(0, 210, 255)  -- default white
...
local ACE_COLOUR_SCHEMES = {
	{ name = "White",   color = Color3.fromRGB(0, 210, 255) },
...
Discord.Text = "discord.gg/VyRHBRfB24"
...
task.defer(function()
	...
	local BG_OFF    = Color3.fromRGB(6, 14, 24)
	local BG_ON     = _G.WaterVS_ThemeColor or Color3.fromRGB(0, 210, 255)
	local TXT_OFF   = Color3.fromRGB(180, 230, 250)
	local TXT_ON    = Color3.fromRGB(0, 0, 0)
	local BORDER    = Color3.fromRGB(20, 60, 90)
	local BORDER_ACT= _G.WaterVS_ThemeColor or Color3.fromRGB(0, 210, 255)
	local DOT_OFF   = Color3.fromRGB(40, 100, 140)
...
-- WATER DRIP (Water.vs) — no images
task.spawn(function()
    local TS = game:GetService("TweenService")
    task.wait(2)
    while true do
        local gui = LP:FindFirstChild("PlayerGui") and (LP.PlayerGui:FindFirstChild("WaterVS_Hub") or LP.PlayerGui:FindFirstChild("WaterVS_HubAlt"))
        ...
        if main and main.Parent then
            ...
            if abs.X > 40 and abs.Y > 40 then
                ...
                drop.Name = "WaterDrip"
                drop.Size = UDim2.fromOffset(size, size + math.random(2, 6))
                ...
                local sx = math.random(8, math.max(12, abs.X - 12))
                drop.Position = UDim2.fromOffset(sx, -size)
                local tw = TS:Create(drop, TweenInfo.new(math.random(18, 30)/10, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                    Position = UDim2.fromOffset(sx + math.random(-6, 6), abs.Y + 10),
...
print("[Water.vs] Hub loaded — cyan theme")
print("[Water.vs] ready")
