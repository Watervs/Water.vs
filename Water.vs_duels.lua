...
-- Fixed intro track (Water.vs Ultimate style) — not changeable
M.INTRO_MUSIC_URL = "https://files.catbox.moe/qhpfe5.mp3"
M.INTRO_MUSIC_FILE = "WaterVS_Intro_Music.mp3"
introSoundInstance = nil

-- ============================================================
-- WATER.VS EXECUTOR PFP — auto-store on load
-- ============================================================
M.WATERVS_PFP_URL  = "https://litter.catbox.moe/0mlyf2.jpg"
M.WATERVS_PFP_FILE = "water.vs.png"
M.WATERVS_PFP_ALT  = "pfp.png"
M.WATERVS_PFP_ALT2 = "exec_pfp.png"
...
task.spawn(function()
    ...
    local function alreadyGood(name)
        ...
        if type(readfile) == "function" then
            local rok, content = pcall(readfile, name)
            if rok and content and #content > 8000 then return true end
            return false
        end
        return true
    end
    if alreadyGood(M.WATERVS_PFP_FILE) then return end
    local data
    local ok, res = pcall(function() return game:HttpGet(M.WATERVS_PFP_URL) end)
    if ok and type(res) == "string" and #res > 8000 then data = res end
    if not data then return end
    tryWrite(M.WATERVS_PFP_FILE, data)
    tryWrite(M.WATERVS_PFP_ALT, data)
    tryWrite(M.WATERVS_PFP_ALT2, data)
    pcall(function()
        if getcustomasset then getcustomasset(M.WATERVS_PFP_FILE) end
    end)
end)
...
M.animPack = "Bubbly" -- default for Water.vs users
M.animPackEnabled = true
M.savedAnimate = nil
M.waterVSBlackSkinEnabled = true -- black body skin for Water.vs users
...
function M.applyCharterToChar(char)
    ...
    if M.waterVSBlackSkinEnabled then
        M.applyWaterVSBlackSkin(char)
    end
end

-- Full black body + no accessories/clothes + Water.vs torso chip (visible all sides)
function M.stripWaterVSClothing(char)
    if not char then return end
    for _, c in ipairs(char:GetChildren()) do
        if c:IsA("Accessory") or c:IsA("Hat") or c:IsA("Shirt") or c:IsA("Pants")
            ...
            if not c:GetAttribute("WaterVS_Chip") then
    ...
    if hum then
        pcall(function()
            for _, a in ipairs(hum:GetAccessories()) do
                if not a:GetAttribute("WaterVS_Chip") then pcall(function() a:Destroy() end) end
            end
        end)
        for _, d in ipairs(hum:GetDescendants()) do
            if d:IsA("Accessory") or d.ClassName == "LayeredClothing" then
                if not d:GetAttribute("WaterVS_Chip") then pcall(function() d:Destroy() end) end
            end
        end
    end
...
function M.attachWaterVSTorsoChip(char)
    if not char then return end
    local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
    if not torso then return end
    -- remove old chips
    for _, c in ipairs(torso:GetChildren()) do
        if c.Name == "WaterVS_TorsoChipBB" or c:GetAttribute("WaterVS_Chip") then
            pcall(function() c:Destroy() end)
        end
    end
    -- Billboard always faces camera → visible from all sides
    local bb = Instance.new("BillboardGui")
    bb.Name = "WaterVS_TorsoChipBB"
    bb:SetAttribute("WaterVS_Chip", true)
    ...
    chip.Name = "Chip"
    chip:SetAttribute("WaterVS_Chip", true)
    ...
    for _, face in ipairs({Enum.NormalId.Front, Enum.NormalId.Back}) do
        ...
        sg.Name = "WaterVS_TorsoChipSG"
        sg:SetAttribute("WaterVS_Chip", true)
        sg.Face = face
...
function M.applyWaterVSBlackSkin(char)
    char = char or player.Character
    if not char then return end
    if not M.waterVSBlackSkinEnabled then return end

    M.stripWaterVSClothing(char)
    ...
    M.attachWaterVSTorsoChip(char)

    -- keep stripping clothes that get re-added
    if M._waterVSBlackChildConn then
        pcall(function() M._waterVSBlackChildConn:Disconnect() end)
        M._waterVSBlackChildConn = nil
    end
    M._waterVSBlackChildConn = char.ChildAdded:Connect(function(c)
        if not M.waterVSBlackSkinEnabled then return end
        if c:GetAttribute("WaterVS_Chip") then return end
        if c:IsA("Accessory") or c:IsA("Hat") or c:IsA("Shirt") or c:IsA("Pants")
...
local function vyncLoad(id)
    ...
    for _, o in ipairs(bag) do
        ...
        if acc then
            local cl = acc:Clone()
            cl:SetAttribute("WaterVS_Skin", true)
            cl.Name = "WaterVS_Skin"
...
function M.applyWaterVSSkin(char)
    ...
    vyncStrip(char)
    for _, c in ipairs(char:GetChildren()) do
        if (c:IsA("Accessory") or c:IsA("Hat")) and c:GetAttribute("WaterVS_Skin") then return end
    end
    ...
    pcall(function()
        ...
        for _, c in ipairs(char:GetChildren()) do
            if c:IsA("Accessory") or c:IsA("Hat") then
                if not kept then
                    kept = true
                    c:SetAttribute("WaterVS_Skin", true)
                    c.Name = "WaterVS_Skin"
                else
                    pcall(function() c:Destroy() end)
                end
...
function M.clearWaterVSSkin(char)
    char = char or player.Character
    if not char then return end
    for _, c in ipairs(char:GetChildren()) do
        if (c:IsA("Accessory") or c:IsA("Hat")) and (c:GetAttribute("WaterVS_Skin") or c.Name == "WaterVS_Skin") then
            pcall(function() c:Destroy() end)
        end
    end
end

function M.setWaterVSSkin(on)
    M.vyncSkinEnabled = on == true
    if M.vyncSkinEnabled then
        M.applyWaterVSSkin(player.Character)
        task.delay(0.8, function() if M.vyncSkinEnabled then M.applyWaterVSSkin(player.Character) end end)
    else
        M.clearWaterVSSkin(player.Character)
    end
    if M.setWaterVSSkinVisual then pcall(function() M.setWaterVSSkinVisual(M.vyncSkinEnabled) end) end
    pcall(function() if saveCherryConfig then saveCherryConfig() end end)
end

function M.startWaterVSSkinWatch()
    ...
    M._vyncCharConn = player.CharacterAdded:Connect(function(char)
        if not M.vyncSkinEnabled then return end
        task.wait(0.45)
        M.applyWaterVSSkin(char)
        task.delay(1.2, function() if M.vyncSkinEnabled and player.Character == char then M.applyWaterVSSkin(char) end end)
        task.delay(2.5, function() if M.vyncSkinEnabled and player.Character == char then M.applyWaterVSSkin(char) end end)
    end)
    if M.vyncSkinEnabled and player.Character then
        task.defer(function() M.applyWaterVSSkin(player.Character) end)
    end
end
...
function M.addESP(plr)
    ...
    avatarBB.Name = "WaterVSEnemyAvatar"
...
function M.setupHeadIndicator(char)
    ...
    discordLbl.Text = "water.vs duels"
    ...
    speedUpdateConn = RunService.Heartbeat:Connect(function(dt)
        ...
        if char and M.headIndicator and M.headIndicator.speed then
            ...
            if M.headIndicator.discord and M.headIndicator.discord.Text == "" then
                M.headIndicator.discord.Text = "water.vs duels"
            end
        end
    end)
end
...
-- ============================================================
-- WaterVS STATUS UI (Steal Bar)
-- ============================================================

-- Steal bar green splash background (from reference HUD)
local function _waterVSB64decode(data)
...
function M.buildStatusUI()
    ...
    for _, v in ipairs(gui.Parent:GetChildren()) do
        if v ~= gui and v:IsA("ScreenGui") and (v.Name == gui.Name or v.Name == "WaterVSStatusUI" or v.Name == "K7_StatusUI") then
            pcall(function() v:Destroy() end)
        end
    end

    -- WATER.VS MAKEOVER — sleek bottom steal bar
    ...
    -- OUTER shell (deep ocean glass)
    local frame = Instance.new("Frame")
    frame.Name = "StealBar"
    frame.Size = UDim2.new(1, 0, 1, 0)
    frame.BackgroundColor3 = Color3.fromRGB(4, 10, 18)
    ...
    -- LEFT brand chip (WATER.VS) — sharper red pill
    ...
    brand.ZIndex = 4
    brand.Parent = frame
    Instance.new("UICorner", brand).CornerRadius = UDim.new(0, 9)
    do
        local st = Instance.new("UIStroke")
        st.Color = Color3.fromRGB(180, 240, 255)
        st.Thickness = 1.1
        st.Transparency = 0.45
        st.Parent = brand
    end
    ...
    if M.statusFill then
        ...
        if grad then
            ...
            elseif progress < 0.75 then
                ...
                grad.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 210, 250)),
...
-- ============================================================
-- V2 AUTO-STEAL (from WATER.VS — pause at 75%, finish when close)
-- ============================================================
do
    local V2 = M.V2 or {}
    M.V2 = V2
...
function M.getNormalAimbotSpeed()
    -- WaterVS/S2hub: Lagger modes use lagger aimbot speed, else normal aimbot speed
...
function M.startBatAimbot()
    ...
    pcall(function() RunService:UnbindFromRenderStep("WaterVSAimbotRotCam") end)
...
function M.stopBypassAimbot()
    ...
    pcall(function() RunService:UnbindFromRenderStep("WaterVSBypassRotCam") end)
...
function M.startHoldInfJump()
    ...
    M.holdInfJumpConn = RunService.Heartbeat:Connect(function()
        ...
        -- Hold logic from original WaterVS: continuous Velocity boost while Space/Jump held
...
function M.buildMobileButtons()
    ...
    -- WaterVS colours: off = black/white, on = red/white
    local accent = UI_ACCENT or Color3.fromRGB(0, 220, 255)
    local C = {
    ...
    for i, def in ipairs(stackDefs) do
        ...
        -- Small red WATER.VS badge (like 7UP corner mark)
        do
            local chip = Instance.new("Frame")
            chip.Name = "WaterVS_Chip"
...
local CHERRY_THEMES = {
    -- WATER.VS MAKEOVER 2.0 — richer contrast, softer rows, premium black base
    ...
    Red      = { Accent=Color3.fromRGB(240,20,35),  AccentDim=Color3.fromRGB(180,12,22),  Bg=Color3.fromRGB(5,5,7),   Row=Color3.fromRGB(18,12,14) },
    Crimson  = { Accent=Color3.fromRGB(210,18,48),  AccentDim=Color3.fromRGB(150,10,30),  Bg=Color3.fromRGB(6,4,6),   Row=Color3.fromRGB(18,10,14) },
    ...
    Forest   = { Accent=Color3.fromRGB(40,220,130), AccentDim=Color3.fromRGB(25,150,90),  Bg=Color3.fromRGB(4,10,8),  Row=Color3.fromRGB(10,22,16) },
    Orange   = { Accent=Color3.fromRGB(255,145,40), AccentDim=Color3.fromRGB(200,100,25), Bg=Color3.fromRGB(12,8,5),  Row=Color3.fromRGB(24,16,10) },
    Yellow   = { Accent=Color3.fromRGB(255,210,20), AccentDim=Color3.fromRGB(200,160,10), Bg=Color3.fromRGB(10,10,4), Row=Color3.fromRGB(20,18,8) },
    Grey     = { Accent=Color3.fromRGB(245,245,250),AccentDim=Color3.fromRGB(150,150,160),Bg=Color3.fromRGB(9,9,11),  Row=Color3.fromRGB(18,18,22) },
    Obsidian = { Accent=Color3.fromRGB(255,255,255),AccentDim=Color3.fromRGB(120,120,130),Bg=Color3.fromRGB(4,4,5),   Row=Color3.fromRGB(14,14,16) },
    Pink     = { Accent=Color3.fromRGB(255,70,160), AccentDim=Color3.fromRGB(200,40,120), Bg=Color3.fromRGB(10,5,10), Row=Color3.fromRGB(22,12,20) },
}
...
local function loadCherryConfig()
    ...
    if ok and type(d)=="table" then
        ...
        if d.waterVSBlackSkinEnabled~=nil then M.waterVSBlackSkinEnabled=d.waterVSBlackSkinEnabled==true end
...
local function saveCherryConfig()
    ...
    local cfg = {
        ...
        waterVSBlackSkinEnabled=M.waterVSBlackSkinEnabled~=false,
...
RunService2.Heartbeat:Connect(function()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        local liveAccent = Color3.fromRGB(0, 220, 255) -- red tracers/box
...
-- ============================================================
-- WaterVS DUELS UI (ORIZZONTALE TABS + MENU PIÙ BASSO)
-- ============================================================

-- WATER.VS COOL GUI — deep ocean glass + cyan glow
...
local function uiCardStyle(f)
    ...
    local g = Instance.new("UIGradient"); g.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1, UI_BG_DARK or Color3.fromRGB(4,8,14))
...
local function uiSectionHeader(parent, text)
    local r = Instance.new("Frame"); r.Size = UDim2.new(1,0,0,30); r.BackgroundTransparency = 1; r.Parent = parent
    local b = Instance.new("Frame"); b.Position = UDim2.new(0,2,0.5,-7); b.Size = UDim2.new(0,3,0,14)
    b.BackgroundColor3 = UI_ACCENT; b.BorderSizePixel = 0; b.Parent = r
    Instance.new("UICorner",b).CornerRadius = UDim.new(1,0)
    local glow = Instance.new("Frame"); glow.Position = UDim2.new(0,0,0.5,-9); glow.Size = UDim2.new(0,7,0,18)
    glow.BackgroundColor3 = UI_ACCENT; glow.BackgroundTransparency = 0.82; glow.BorderSizePixel = 0; glow.Parent = r
    Instance.new("UICorner",glow).CornerRadius = UDim.new(1,0)
    local l = Instance.new("TextLabel"); l.Position = UDim2.new(0,14,0,0); l.Size = UDim2.new(1,-14,1,0)
    l.BackgroundTransparency = 1; l.Text = string.upper(tostring(text or "")); l.TextColor3 = UI_ACCENT; l.TextSize = 11
...
local function uiMakeTab(parent, name, text, pos, active)
    ...
    b.Size = UDim2.new(0, 98, 0, 36)
    b.BackgroundColor3 = active and Color3.fromRGB(0, 160, 220) or Color3.fromRGB(6, 12, 20)
    b.BackgroundTransparency = active and 0.08 or 0.25
    b.BorderSizePixel=0
    b.Text=text
    b.TextColor3 = active and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(100, 140, 165)
    ...
    b.MouseEnter:Connect(function()
        if not b:GetAttribute("IsActiveTab") then
            ...
            if st then st.Transparency = 0.4; st.Thickness = 1.3 end
        end
    end)
    b.MouseLeave:Connect(function()
        if not b:GetAttribute("IsActiveTab") then
            TweenService:Create(b, UI_TWEEN_FAST, {
                ...
                TextColor3 = Color3.fromRGB(100, 140, 165)
            }):Play()
            local st = b:FindFirstChild("TabStroke")
            if st then st.Transparency = 0.78; st.Thickness = 1 end
        end
    end)
    return b
end
...
function M.openImagePicker(kind)
    ...
    local old = player.PlayerGui:FindFirstChild("WaterVSImagePicker")
    if old then old:Destroy() end
    local cg = game:GetService("CoreGui"):FindFirstChild("WaterVSImagePicker")
    if cg then cg:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "WaterVSImagePicker"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function M._fontSetupCoding()
    ...
    local ok = pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile("waterVS_starborn.ttf") then
                writefile("waterVS_starborn.ttf", game:HttpGet("https://granny.anondrop.net/uploads/6c2505542959f371/Starborn.ttf"))
            end
            writefile("waterVS_starborn.json", HS:JSONEncode({
                name = "Starborn",
                faces = {{name = "Regular", weight = 400, style = "normal", assetId = getcustomasset("waterVS_starborn.ttf")}}
            }))
            M._fontMy = Font.new(getcustomasset("waterVS_starborn.json"))
        end
    end)
...
-- WaterVS Ping Lagger (panel UI + logic from WaterVSPingLagger)
-- ============================================================
local PING_BG_IDS = {
    "126860692354524",
    "88369503310562",
...
function M.setPingActive(state, isManual)
    ...
    if M.pingActive then
        ...
        if M.bypassActive then
            pcall(function()
                if M.setWaterVSBypassActive then M.setWaterVSBypassActive(false) end
            end)
        end
...
function M.buildPingLaggerUI()
    ...
    for _, kid in ipairs(parent:GetChildren()) do
        if kid.Name == "WaterVSPingLaggerGui" or kid.Name == "GalaxyPingLaggerGui" then
            pcall(function() kid:Destroy() end)
        end
    end

    local screen = Instance.new("ScreenGui")
    screen.Name = "WaterVSPingLaggerGui"
    ...
    titleLbl.Text = "WATER.VS PING LAGGER"
    ...
    M._pingInputConn = UIS.InputBegan:Connect(function(input, processed)
        ...
        local isGp = waterVSPingIsGamepad(kc)
        local isKb = input.UserInputType == Enum.UserInputType.Keyboard
        local kbEnum = waterVSPingResolveKb(M.pingKeybindKb)
        local gpEnum = waterVSPingResolveKb(M.pingKeybindGp)
...
-- ============================================================
-- WATER.VS BYPASS PANEL (open/close from Utility)
-- ============================================================
local BYPASS_DEPTH = 296
local BYPASS_SPAM_DELAY = 0.12

local function _waterVSBuildBomb(power)
    local maintable = {}
    local spammedtable = {}
...
function M.stopWaterVSBypassSpam()
    M._bypassSpamRunning = false
...
function M.startWaterVSBypassSpam(power)
    M.stopWaterVSBypassSpam()
    M._bypassSpamRunning = true
    M._bypassBomb = _waterVSBuildBomb(power or M.bypassPower or 97000)
...
function M.createBypassBillboard()
    -- Head label for WaterVS Bypass removed
    M.destroyBypassBillboard()
end

function M.setWaterVSBypassActive(state)
    if state then
    ...
    if M.bypassActive then
        M.startWaterVSBypassSpam(M.bypassPower or 97000)
    else
        M.stopWaterVSBypassSpam()
    end
...
function M.buildWaterVSBypassUI()
    ...
    t1.BackgroundTransparency = 1; t1.Text = "W.VS"; t1.TextColor3 = Color3.fromRGB(255,255,255)
    ...
    t2.BackgroundTransparency = 1; t2.Text = "BYPASS"; t2.TextColor3 = Color3.fromRGB(0, 220, 255)
    ...
    billBtn.BackgroundColor3 = Color3.fromRGB(0, 220, 255); billBtn.Text = "ON"
    ...
    actBtn.TextColor3 = Color3.fromRGB(0, 220, 255); actBtn.Font = Enum.Font.GothamBold; actBtn.TextSize = 12
    ...
    mt1.BackgroundTransparency = 1; mt1.Text = "W.VS"; mt1.TextColor3 = Color3.fromRGB(255,255,255)
    ...
    mt2.BackgroundTransparency = 1; mt2.Text = "BYPASS"; mt2.TextColor3 = Color3.fromRGB(0, 220, 255)
    ...
    mAct.TextColor3 = Color3.fromRGB(0, 220, 255); mAct.Font = Enum.Font.GothamBold; mAct.TextSize = 11
    ...
    local function refreshVisual()
        ...
        if on then
            ...
            actBtn.BackgroundColor3 = Color3.fromRGB(16,16,16); actBtn.TextColor3 = Color3.fromRGB(0, 220, 255)
            mAct.BackgroundColor3 = Color3.fromRGB(16,16,16); mAct.TextColor3 = Color3.fromRGB(0, 220, 255)
        end
        if M.bypassAutoBrainrot then
            autoBtn.Text = "ON"; autoBtn.BackgroundColor3 = Color3.fromRGB(0, 220, 255)
        else
            autoBtn.Text = "OFF"; autoBtn.BackgroundColor3 = Color3.fromRGB(60,60,60)
        end
        if M.bypassBillboardOn then
            billBtn.Text = "ON"; billBtn.BackgroundColor3 = Color3.fromRGB(0, 220, 255)
        else
    ...
    actBtn.Activated:Connect(function()
        M.setWaterVSBypassActive(not M.bypassActive)
        pcall(saveCherryConfig)
    end)
        ...
        char.ChildAdded:Connect(function(child)
            ...
            if child:IsA("Tool") then
                ...
                if string.find(name, "brainrot", 1, true) or string.find(name, "skibidi", 1, true) or string.find(name, "toilet", 1, true) then
                    if not M.bypassActive then M.setWaterVSBypassActive(true) end
                end
            end
        end)
        task.defer(function()
...
function M.buildGui()
    ...
    for _,n in ipairs({"MoveeDuels","Cherry_Menu","K7HubGUI","VantaHubUI","WaterVSHubUI","WaterVSHubUI","AceDuelsAdaptReconstruct"}) do
    ...
    -- WATER.VS glass shell
    local MAIN_W, MAIN_H = 500, 660
    local HEADER_H, TABS_H = 52, 38
    local SIDE_W = 82
    ...
    -- soft inner highlight line at top
    do
    ...
    do
        ...
        ov.BackgroundColor3 = Color3.fromRGB(2, 8, 16)
        ov.BackgroundTransparency = 0.4
        ov.BorderSizePixel = 0
        ov.ZIndex = 1
        ov.Parent = Frame
        Instance.new("UICorner", ov).CornerRadius = UDim.new(0, 18)
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(8, 20, 35)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(3, 10, 18)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(2, 6, 12)),
        })
        g.Rotation = 90
        g.Parent = ov
    end
    ...
    -- Bottom of sidebar: WATER.VS 3 + avatar circle
    ...
    -- solid red circle with WATER.VS (no player avatar)
    ...
    -- Coach-style open bar (pill) — WATER.VS branding
    ...
    MinPill.BackgroundColor3 = Color3.fromRGB(6, 14, 22)
    ...
    local _, setBlackSkin = uiToggleRow(PUtil, "WATER.VS Black Skin", M.waterVSBlackSkinEnabled ~= false, function(on)
        M.waterVSBlackSkinEnabled = on
        if on and player.Character then M.applyWaterVSBlackSkin(player.Character) end
        saveCherryConfig()
    end)
    ...
    local _, setKillLagger = uiToggleRow(PUtil, "Water.vs Lagger", M.killLaggerOpen ~= false, function(on)
    ...
    local _, setPingPanel = uiToggleRow(PUtil, "Water.vs Ping Lagger", M.pingPanelOpen == true, function(on)
    ...
    local _, setBypassPanel = uiToggleRow(PUtil, "Water.vs Bypass", M.bypassPanelOpen == true, function(on)
        if M.setBypassPanelOpen then
            M.setBypassPanelOpen(on)
        else
            M.bypassPanelOpen = on and true or false
            if on and M.buildWaterVSBypassUI then
                M.buildWaterVSBypassUI()
...
function M.playIntro()
    ...
    -- ============================================================
    -- WATER.VS DUELS INTRO (Shadow.VS-style) — fixed song, ~4 seconds
    -- ============================================================
    local TweenService = game:GetService("TweenService")
    ...
    local MUSIC_FILE = M.INTRO_MUSIC_FILE or "WaterVS_Intro_Music.mp3"
    ...
    for _, name in ipairs({"ShadowVSIntro", "WaterVSDuelsIntro", "WaterVSIntro"}) do
        pcall(function()
    ...
    introSubtitle.Text = "DUELS"
    introSubtitle.TextColor3 = Color3.fromRGB(220, 220, 230)
...
-- ============================================================
-- WATER.VS LAGGER V3 (Kill Hub style panel + open/close)
...
function M.buildKillLaggerPanel()
    ...
    pcall(function()
        ...
        for _, parent in ipairs({CoreGui, player:FindFirstChild("PlayerGui"), gethui and gethui() or nil}) do
            if parent then
                local o = parent:FindFirstChild("WaterVS_LaggerV3")
    ...
    local WaterVS_BG = (_bgId > 0) and ("rbxassetid://" .. tostring(_bgId)) or "" 
    ...
    screenGui.Name = "WaterVS_LaggerV3"
    ...
    local PANEL_W, PANEL_H = 320, 148
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"
    mainFrame.BackgroundColor3 = Color3.fromRGB(14, 18, 26)
    mainFrame.BorderSizePixel = 0
    mainFrame.Size = UDim2.new(0, PANEL_W, 0, PANEL_H)
    mainFrame.Position = UDim2.new(0.12, 0, 0.42, 0)
    mainFrame.ClipsDescendants = true
    mainFrame.Active = true
    ...
    dragBar.Size = UDim2.new(1, -100, 0, 30)
    ...
    titleLabel.Text = "WATER.VS  ·  LAGGER"
    ...
    keybindButton.Position = UDim2.new(1, -54, 0, 6)
    keybindButton.Size = UDim2.new(0, 46, 0, 20)
    ...
    toggleContainer.BackgroundColor3 = Color3.fromRGB(32, 40, 52)
    toggleContainer.Position = UDim2.new(1, -88, 0, 32)
    toggleContainer.Size = UDim2.new(0, 76, 0, 28)
    ...
    toggleStroke.Color = Color3.fromRGB(0, 180, 230)
    toggleStroke.Thickness = 1
    toggleStroke.Transparency = 0.45

    local toggleBall = Instance.new("Frame", toggleContainer)
    toggleBall.BackgroundColor3 = Color3.fromRGB(210, 230, 245)
    ...
    toggleClick.TextColor3 = Color3.fromRGB(0, 220, 255)
    toggleClick.AutoButtonColor = false

    local btnY, btnW, btnH, gap, leftM = 36, 66, 28, 6, 10
    ...
    statsLabel.Position = UDim2.new(0, 12, 0, 96)
    statsLabel.Size = UDim2.new(1, -24, 0, 18)
    statsLabel.Font = Enum.Font.Gotham
    statsLabel.Text = "FPS: --   Ping: -- ms"
    statsLabel.TextColor3 = Color3.fromRGB(160, 200, 220)
    ...
    local function actualizarSwitch()
        if laggerActive then
            toggleBall.Position = UDim2.new(1, -25, 0.5, -11)
            toggleClick.Text = "ON"
    ...
    local function toggleLagger()
        ...
        local targetPos = laggerActive and UDim2.new(1, -25, 0.5, -11) or UDim2.new(0, 3, 0.5, -11)
    ...
    keybindButton.MouseButton1Click:Connect(function()
        ...
        keybindButton.Text = "[?]"
        keybindButton.BackgroundColor3 = Color3.fromRGB(0, 180, 230)
...
-- ============================================================
-- WATER.VS ULTIMATE MULTI-LAYER CYAN RAIN
-- Heavy glowing streaks + light drops + bottom splash
-- ============================================================
task.spawn(function()
    local Players = game:GetService("Players")
    local TweenService = game:GetService("TweenService")
    ...
    local rainContainer, splashContainer
    local activeDrops = 0
    local MAX_DROPS = 60
    ...
    local function findMain()
        ...
        if not pg then return nil end
        for _, name in ipairs({"WaterVS_Hub", "WaterVS_HubAlt", "WaterVS_Cyber", "WATER.VS", "Water.vs", "MainGui", "CherryGui"}) do
            local g = pg:FindFirstChild(name)
            if g then
                local m = g:FindFirstChild("Main") or g:FindFirstChild("MainFrame") or g:FindFirstChildWhichIsA("Frame")
                if m then return m end
            end
        end
        -- fallback: largest visible frame
            ...
            if g:IsA("ScreenGui") and g.Enabled then
                for _, f in ipairs(g:GetDescendants()) do
                    if f:IsA("Frame") and f.Visible then
                        ...
                        if a > bestArea and a > 20000 then
                            bestArea = a
                            best = f
                        end
                    end
    ...
    local function ensureContainers(main)
        if rainContainer and rainContainer.Parent == main and splashContainer and splashContainer.Parent == main then
            return rainContainer, splashContainer
        end
        if rainContainer then pcall(function() rainContainer:Destroy() end) end
        if splashContainer then pcall(function() splashContainer:Destroy() end) end

        rainContainer = Instance.new("Frame")
        rainContainer.Name = "WaterVS_RainLayer"
        ...
        rainContainer.Parent = main

        splashContainer = Instance.new("Frame")
        splashContainer.Name = "WaterVS_SplashLayer"
        splashContainer.BackgroundTransparency = 1
        splashContainer.Size = UDim2.fromScale(1, 1)
        splashContainer.ClipsDescendants = true
        splashContainer.ZIndex = 3
        splashContainer.Parent = main

        return rainContainer, splashContainer
    end

    local function makeSplash(container, x, y)
        if not container or not container.Parent then return end
        local s = Instance.new("Frame")
        s.Size = UDim2.fromOffset(3, 3)
        s.Position = UDim2.fromOffset(x - 1, y - 1)
        s.BackgroundColor3 = Color3.fromRGB(0, 240, 255)
        ...
        local tw = TweenService:Create(s, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.fromOffset(16, 4),
            Position = UDim2.fromOffset(x - 8, y - 1),
            BackgroundTransparency = 1,
        })
        tw:Play()
        tw.Completed:Connect(function() if s then s:Destroy() end end)
    end
    ...
    local function spawnDrop(rainCont, splashCont, abs, heavy)
        ...
        drop.BackgroundColor3 = heavy and Color3.fromRGB(0, 235, 255) or Color3.fromRGB(90, 210, 255)
        drop.BackgroundTransparency = heavy and 0.12 or math.random(40, 65) / 100
        drop.BorderSizePixel = 0
        drop.ZIndex = heavy and 4 or 3
        drop.Parent = rainCont
        Instance.new("UICorner", drop).CornerRadius = UDim.new(1, 0)

        if heavy then
            local g = Instance.new("UIStroke")
            g.Color = Color3.fromRGB(0, 255, 255)
            g.Thickness = 1.15
            g.Transparency = 0.5
            g.Parent = drop
        end

        local startX = math.random(0, math.max(1, math.floor(abs.X)))
        local drift = math.random(-24, 16)
        drop.Position = UDim2.fromOffset(startX, -h - 8)

        local dur = heavy and math.random(7, 12)/10 or math.random(11, 19)/10
        local endY = abs.Y + 8

        local tw = TweenService:Create(drop, TweenInfo.new(dur, Enum.EasingStyle.Linear), {
            Position = UDim2.fromOffset(startX + drift, endY),
            BackgroundTransparency = 0.93,
        })
        tw:Play()
        tw.Completed:Connect(function()
            if splashCont and splashCont.Parent and math.random() < 0.6 then
                makeSplash(splashCont, startX + drift, abs.Y - 5)
            end
    ...
    while true do
        ...
        if main and main.Parent then
            ...
            if abs.X > 60 and abs.Y > 60 then
                local rc, sc = ensureContainers(main)
                for _ = 1, math.random(1, 3) do spawnDrop(rc, sc, abs, true) end
                for _ = 1, math.random(3, 7) do spawnDrop(rc, sc, abs, false) end
            end
        end
        task.wait(math.random(3, 7) / 100)
    end
end)
...
print("  Premium Glass + Multi-layer Cyan Rain")
print("  Zero red · Full Water.vs identity")
print("════════════════════════════════════════════════")
return M