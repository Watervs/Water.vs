...
-- [[ WATER.VS DUELS — MAKEOVER 2.0 ]]
-- Premium glass UI · new themes · cleaner steal bar · auto water.vs executor PFP on load
...
-- Safe early stub (real impl later)
local function saveCherryConfig() end
...
M.Steal = {
    ...
    StealRadius = 60,
    StealDuration = 0.05, -- lowest practical
    StopTime = 0.05,
}
M.V3 = {
    enabled = false,
...
function M.buildStatusUI()
    ...
    -- VYNX MAKEOVER — sleek bottom steal bar
    ...
    -- LEFT brand chip (VYNX) — sharper red pill
...
do
    ...
    local function execStealV2(prompt)
        ...
        V2.pauseStarted = nil
        V2.pausedDuration = 0
        local duration = math.max(tonumber(M.Steal.StealDuration) or 0.05, 0.05)
...
do
    ...
    local function executeSemi(prompt, animalData)
        ...
        task.spawn(function()
            ...
            while A.enabled and M.stealMode == "Semi" and tick() - startTime < (A.holdMin or 0.05) do
                local elapsed = tick() - startTime
                A.state.phase = "holding"
                barSet(elapsed / (A.holdMax or 0.12), "HOLDING " .. tostring(A.state.label))
                task.wait()
            end
            ...
            while A.enabled and M.stealMode == "Semi" and prompt.Parent do
                local elapsed = tick() - startTime
                if elapsed > (A.holdMax or 0.12) then break end
                barSet(elapsed / (A.holdMax or 0.12), "MOVE CLOSER  " .. tostring(A.state.label))
...
function M.startV3Steal()
    ...
    M.V3.conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local stopT = math.max(tonumber(M.Steal.StopTime) or 0.05, 0.05)
        local holdT = math.max(tonumber(M.Steal.StealDuration) or 0.05, 0.05)
...
function M._adProtectCharacter(char)
    ...
    -- Keep critical parts present
    pcall(function()
        for _, name in ipairs({"Head", "HumanoidRootPart", "UpperTorso", "Torso"}) do
            local p = char:FindFirstChild(name)
            if p and p:IsA("BasePart") then
                p.CanCollide = (name == "HumanoidRootPart") and p.CanCollide or p.CanCollide
                p.Anchored = false
            end
        end
    end)
    ...
    M._antiDieHealthConn = hum:GetPropertyChangedSignal("Health"):Connect(function()
        ...
        pcall(function()
            hum.MaxHealth = math.huge
            if hum.Health < math.huge or hum.Health <= 0 then
                hum.Health = math.huge
            end
            hum.PlatformStand = false
            hum.Sit = false
        ...
        pcall(function()
            ...
            for i = 1, 5 do
                task.delay(0.03 * i, function()
                    if not M.antiDieEnabled or not hum.Parent then return end
                    pcall(function()
                        hum.MaxHealth = math.huge
                        hum.Health = math.huge
                        hum.PlatformStand = false
...
function M.buildMobileButtons()
    ...
    for i, def in ipairs(stackDefs) do
        ...
        if mobBgId > 0 then
            ...
            Instance.new("UICorner", bgImg).CornerRadius = cornerR
...
local CHERRY_THEMES = {
    -- VYNX MAKEOVER 2.0 — richer contrast, softer rows, premium black base
...
function M.applyCustomBackground(frame)
    ...
    pcall(function()
        for _, d in ipairs(frame:GetDescendants()) do
            if d.Name == "CustomBgImage" or d.Name == "BarBgImage" or d.Name == "CustomBackground" then
                d:Destroy()
            end
        end
    end)
...
function M.buildVynxBypassUI()
    ...
    local function refreshVisual()
        ...
        if on then
            actBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 255); actBtn.TextColor3 = Color3.fromRGB(255,255,255)
            mAct.BackgroundColor3 = Color3.fromRGB(0, 200, 255); mAct.TextColor3 = Color3.fromRGB(255,255,255)
        else
...
function M.buildGui()
    ...
    UIScale.Scale = M.uiScale or 0.75
    ...
    M.radInput = srBox
    local _, sdBox = uiNumberRow(v2Box, "Hold Duration", M.Steal.StealDuration, 0.05, 10, function(v)
    ...
    local _, semiHoldMin = uiNumberRow(semiBox, "Hold Min", M.Semi.holdMin or 0.05, 0.05, 5, function(v) M.Semi.holdMin = v end)
    local _, semiHoldMax = uiNumberRow(semiBox, "Hold Max", M.Semi.holdMax or 0.12, 0.05, 8, function(v) M.Semi.holdMax = v end)
    ...
    local _, menuScaleBox = uiNumberRow(PUtil, "Menu Scale", M.uiScale, 0.35, 1.5, function(v)
        M.uiScale = v
    ...
    local _, setVyncSkin = uiToggleRow(PUtil, "Water.vs Skin", M.vyncSkinEnabled == true, function(on)
        M.setVyncSkin(on)
    end)
    ...
    local _, setBlackSkin = uiToggleRow(PUtil, "Water.vs Black Skin", M.vynxBlackSkinEnabled ~= false, function(on)
...
function M.buildKillLaggerPanel()
    ...
    pcall(function()
        ...
        for _, parent in ipairs({CoreGui, player:FindFirstChild("PlayerGui"), gethui and gethui() or nil}) do
            if parent then
                local o = parent:FindFirstChild("WaterVSLaggerV3")
    ...
    screenGui.Name = "WaterVSLaggerV3"
    ...
    local PANEL_W, PANEL_H = 300, 130
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"
    mainFrame.BackgroundColor3 = Color3.fromRGB(8, 16, 26)
    mainFrame.BorderSizePixel = 0
    ...
    headerBar.BackgroundColor3 = Color3.fromRGB(6, 14, 24)
    ...
    headerFill.BackgroundColor3 = Color3.fromRGB(6, 14, 24)
    headerFill.BorderSizePixel = 0
    headerFill.ZIndex = 10

    local dragBar = Instance.new("TextButton", mainFrame)
    dragBar.Name = "DragBar"
    dragBar.Size = UDim2.new(1, -120, 0, 32)
    ...
    toggleContainer.BackgroundColor3 = Color3.fromRGB(28, 40, 55)
    toggleContainer.Position = UDim2.new(1, -112, 0, 6)
    toggleContainer.Size = UDim2.new(0, 56, 0, 20)
    toggleContainer.ZIndex = 50
    Instance.new("UICorner", toggleContainer).CornerRadius = UDim.new(1, 0)
    do
        local ts = Instance.new("UIStroke", toggleContainer)
        ts.Color = Color3.fromRGB(0, 210, 255)
        ts.Thickness = 1.2
    ...
    keybindButton.BackgroundColor3 = Color3.fromRGB(28, 40, 55)
    keybindButton.Position = UDim2.new(1, -50, 0, 6)
    keybindButton.Size = UDim2.new(0, 40, 0, 20)
    keybindButton.Font = Enum.Font.GothamBold
    keybindButton.Text = "[" .. (keybind.Name:gsub("Button","")) .. "]"
    keybindButton.TextColor3 = Color3.fromRGB(220, 230, 240)
    keybindButton.TextSize = 10
    keybindButton.AutoButtonColor = false
    keybindButton.Active = true
    keybindButton.ZIndex = 50
    Instance.new("UICorner", keybindButton).CornerRadius = UDim.new(0, 6)

    local btnY, btnW, btnH, gap, leftM = 40, 65, 28, 6, 12
    local function makeLvl(name, x)
        ...
        b.TextSize = 11
        b.AutoButtonColor = false
        b.BackgroundColor3 = Color3.fromRGB(28, 40, 55)
        b.BorderSizePixel = 0
        b.ZIndex = 15
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 7)
        return b
    ...
    infoLabel.Position = UDim2.new(0, 12, 0, 74)
    ...
    statsLabel.TextColor3 = Color3.fromRGB(160, 190, 210)
    ...
    local function actualizarSwitch()
        if laggerActive then
            toggleBall.Position = UDim2.new(1, -17, 0.5, -7)
            toggleClick.Text = "ON"
            toggleClick.TextColor3 = Color3.fromRGB(255, 255, 255)
            toggleContainer.BackgroundColor3 = Color3.fromRGB(0, 180, 230)
        else
            toggleBall.Position = UDim2.new(0, 3, 0.5, -7)
            toggleClick.Text = "OFF"
            toggleClick.TextColor3 = Color3.fromRGB(0, 220, 255)
    ...
    local function toggleLagger()
        ...
        local targetPos = laggerActive and UDim2.new(1, -17, 0.5, -7) or UDim2.new(0, 3, 0.5, -7)
...
task.spawn(function()
    task.wait(0.35)
    pcall(function()
        if type(M.playIntro) == "function" then M.playIntro() end
    end)
end)
pcall(function()
...
print("WATER.VS DUELS — cyan UI loaded.")
return M